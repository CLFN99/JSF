...
public class KochFractal extends Observable {
    ...
    public Edge generateLeftEdge() {
        hue = 0f;
        cancelled = false;
        Edge e = drawKochEdge(0.5, 0.0, (1 - Math.sqrt(3.0) / 2.0) / 2, 0.75, level);
        return e;
    }
    ...
    public Edge generateBottomEdge() {
        hue = 1f / 3f;
        cancelled = false;
        Edge e = drawKochEdge((1 - Math.sqrt(3.0) / 2.0) / 2, 0.75, (1 + Math.sqrt(3.0) / 2.0) / 2, 0.75, level);
        return e;
    }
    ...
    public Edge generateRightEdge() {
        hue = 2f / 3f;
        cancelled = false;
        Edge e = drawKochEdge((1 + Math.sqrt(3.0) / 2.0) / 2, 0.75, 0.5, 0.0, level);
        return e;
    }
